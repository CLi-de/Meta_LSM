...
class MAML:
    ...
    def construct_model(self, input_tensors_input=None, input_tensors_label=None, prefix='metatrain_'):
        ...
        if 'train' in prefix:
            self.total_loss1 = total_loss1 = tf.reduce_sum(input_tensor=lossesa) / tf.cast(FLAGS.meta_batch_size, dtype=tf.float32)  # total loss的均值,finn论文中的pretrain（对比用）
            ...
            self.total_losses2 = total_losses2 = [tf.reduce_sum(input_tensor=tf.multiply(tf.nn.softmax(w), tf.reduce_sum(input_tensor=lossesb[j], axis=1)))
            ...
            self.metaval_total_loss1 = total_loss1 = tf.reduce_sum(input_tensor=lossesa) / tf.cast(FLAGS.meta_batch_size, dtype=tf.float32)  # inner loss
            self.metaval_total_losses2 = total_losses2 = [tf.reduce_sum(input_tensor=lossesb[j]) / tf.cast(FLAGS.meta_batch_size, dtype=tf.float32) for j in range(num_updates)]  # outer losses(每次迭代的)
