...
class UnsupervisedDBN(BaseEstimator, TransformerMixin, BaseModel):
    ...
    def fit(self, X, weights, reconstruction_error):
        """
        ...
        for rbm in self.rbm_layers:
            rbm.fit(input_data, weights, reconstruction_error)
...
class AbstractSupervisedDBN(BaseEstimator, BaseModel):
    ...
    def fit(self, X, weights, reconstruction_error, pre_train=True):
        """
        Fits a model given data.
        :param X: array-like, shape = (n_samples, n_features)
    ...
    def pre_train(self, X, weights, reconstruction_error):
        """
        Apply unsupervised network pre-training.
        :param X: array-like, shape = (n_samples, n_features)
        :return:
        """
        self.unsupervised_dbn.fit(X, weights, reconstruction_error)
        return self
