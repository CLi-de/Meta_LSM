...
def _initialize_softmax_weights(input_units, output_units):  # output_units: class_num
    ...
    weight['w'] = weight_variable(tf.truncated_normal, [input_units, output_units], stddev)
...
def build_and_train_SVmodel(X, Y, weights_initial, input_units, n_iter_backprop, batch_size, p):
    with tf.variable_scope('weights2merge'):
        num_classes = _determine_num_output_neurons(Y)  # 计算类型数目
        ############################################## build ##########################################################
        visible_units_placeholder = tf.placeholder(tf.float32, shape=[None, input_units])
        keep_prob = tf.placeholder(tf.float32)
        visible_units_placeholder_drop = tf.nn.dropout(visible_units_placeholder, keep_prob)
        ...
        for layer_index in range(len(weights_initial)):  # 这里需要改变激活函数
            activation = tf.nn.relu(
                tf.transpose(tf.matmul(weights_initial[layer_index]['w'], tf.transpose(activation)))
                + weights_initial[layer_index]['b'])
            keep_prob = tf.placeholder(tf.float32)  # 每一层使用不同的keep_prob
        ...
        y_ = tf.placeholder(tf.float32, shape=[None, num_classes])  # 标记值

        output = tf.nn.softmax(y)
        cost_function = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(logits=output, labels=y_))
        #tf.nn.softmax_cross_entropy_with_logits_v2(logits=y, labels=tf.stop_gradient(y_)))
        cost=tf.summary.scalar('cost', cost_function)  # 用于显示
        train_step = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(cost_function)  # 计算gradient并更新
        ############################################## train ##########################################################
        with tf.Session() as sess:
            # summaries合并
            merged = tf.summary.merge([cost])
            # 写到指定的磁盘路径中
            ...
            sess.run(tf.initialize_variables(var_list= init))
            ...
            # 输出权值信息
            with tf.variable_scope('weights_for_predicting'):
                SV_weights = []
                ...
                temp_dict = dict()
                temp_dict['w'] = sess.run(tf.transpose(softmax_weight['w']))  # 'w'为Tensor而非Variable
