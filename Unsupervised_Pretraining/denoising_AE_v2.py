...
class AdditiveGaussianNoiseAutoencoder(object):
    def __init__(self,n_input,n_hidden,transfer_function=tf.nn.relu,
                 optimizer = tf.compat.v1.train.AdamOptimizer(),scale=0.1):
        ...
        self.hidden=self.transfer(tf.add(tf.matmul(self.x+scale * tf.random.normal((n_input,)),self.weights['w1']),self.weights['b1']))
        self.reconstruction=tf.add(tf.matmul(self.hidden,self.weights['w2']),self.weights['b2'])
        #      接下来定义自编码器的损失函数，这里使用平方误差作为损失函数，再定义训练操作作为优化器对损失进行优化，最后创建Session并初始化自编码器的全部模型参数。
        self.cost=0.5 * tf.reduce_sum(input_tensor=tf.pow(tf.subtract(self.reconstruction,self.x),2.0))
        self.optimizer=optimizer.minimize(self.cost)
        #init=tf.global_variables_initializer()
        local_variables = tf.compat.v1.global_variables(scope='DAE')
        global_variables = tf.compat.v1.global_variables()
        init=tf.compat.v1.variables_initializer(var_list=tf.compat.v1.global_variables(scope='DAE'))
