...
def init_weights(file):
    ...
    with tf.compat.v1.variable_scope('model'):  # get variable in 'model' scope, to reuse variables
        ...
        weights = {}
        weights['w1'] = tf.Variable(npzfile['arr_0'])
        weights['b1'] = tf.Variable(npzfile['arr_1'])
        weights['w2'] = tf.Variable(npzfile['arr_2'])
        weights['b2'] = tf.Variable(npzfile['arr_3'])
        weights['w3'] = tf.Variable(npzfile['arr_4'])
        weights['b3'] = tf.Variable(npzfile['arr_5'])
        weights['w4'] = tf.Variable(npzfile['arr_6'])
        weights['b4'] = tf.Variable(npzfile['arr_7'])
    return weights
...
# initialize with npzfile of certain subtask(k)
i = 50  # ith subtask
