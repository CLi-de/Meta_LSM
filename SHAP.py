...
tmp_ = np.hstack((tmp[1:, :-3], tmp[1:, -1].reshape(-1, 1))).astype(np.float16)
np.random.shuffle(tmp_)  # shuffle
# 训练集
x_train = tmp_[:int(tmp_.shape[0] / 100), :-1]  # 加载i行数据部分
y_train = tmp_[:int(tmp_.shape[0] / 100), -1]  # 加载类别标签部分
x_train = x_train / x_train.max(axis=0)
# 测试集
x_test = tmp_[int(tmp_.shape[0] / 100): int(tmp_.shape[0] / 50), :-1]  # 加载i行数据部分
y_test = tmp_[int(tmp_.shape[0] / 100): int(tmp_.shape[0] / 50), -1]  # 加载类别标签部分
...
explainer = shap.KernelExplainer(model.predict_proba, shap.kmeans(x_train, 80))
shap_values = explainer.shap_values(x_test, nsamples=80)  # shap_values(_prob, n_samples, features)
...
shap.dependence_plot('slope', shap_values[1], x_test, interaction_index=None)
shap.dependence_plot('slope', shap_values[1], x_test, interaction_index='DEM')
